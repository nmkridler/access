...
def main():
	...
	#X, y = np.array(pd.read_csv('../data/cprobQuadsTrain.csv')), np.array(pd.read_csv('../data/train.csv').ACTION)
	...
	params = {'max_depth':8, 'subsample':0.5, 'verbose':0, 'random_state':0,
	...
	#c.validate(clf,nFolds=10,out='subs/gbmWRawTrain2.csv')
	...
	out.ACTION = y_
	out.to_csv('subs/gbmWRawTest.csv',index=False)
